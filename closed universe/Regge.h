...
public:
    ...
    double get_rad1() const {return (rad+block_ln)*Schw_dr+R;}
    ...
    //Following functions specific to closed universe.
    double t_to_L(const double time, const double posn[], const double traj[]) const;
...
private:
	...
	unsigned long int rad,	//"rad" stores block's radius in units of Schw_dr, with zero corresponding to horizon.
					  block_ln;	//radial size of block in units of Schw_dr;
    ...
    void fwd_blockln();
	inline unsigned long int calc_blockln(const double step) const;
